...
/**
 * @title Simple Yield Circle Factory (Test Version)
 * @dev Simplified factory contract for creating yield circles with test random generation
 * @author Yield Circle Team
 */
contract SimpleYieldCircleFactoryTest is AccessControl, Pausable {
        ...
        returns (address circleAddress, uint256 requestId)
    {
        ...
        // Use test random positions instead of VRF
        uint256[] memory positions = randomGenerator.generateTestPositions(members.length);
        requestId = 0; // No VRF request ID for test version
